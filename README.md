...
ETFの7日・30日合計は、**最新観測日を含む7暦日・30暦日**を集計します。土日は架空のゼロを追加しません。平日の記録に欠損がある場合は合計を取得不可とします。米国市場の休場日を自動判定しないため、祝日の記録がない期間も保守的に取得不可となります。
...
- Glassnodeの取得失敗は同じ取得元の過去値を現在値から除外します。再取得に成功した場合も、鮮度は元の観測日で判定します。
- Coin Metricsの当日分がまだ公開されていない場合は公開待ちとし、最後に公表された値を元の観測日と期限のまま使用します。過去の日付の欠損や不正値は補完しません。
