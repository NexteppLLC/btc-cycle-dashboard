...
def main():
    ...
    args.end = args.end or date.today(); args.start = args.start or args.end - timedelta(days=3652)
    if args.start > args.end: parser.error("--start must not be after --end")
    if args.chunk_days < 1: parser.error("--chunk-days must be positive")
    if args.sleep < 0: parser.error("--sleep must be nonnegative")
    create_schema(); settings = get_settings()
    if args.asset == "btc": collectors = [OnChainCollector(), GlassnodeCollector(settings.glassnode_api_key)]
    else:
