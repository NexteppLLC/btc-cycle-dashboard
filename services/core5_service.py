...
def build_core5(metrics, snapshots, cfg: dict, as_of: date | None = None) -> dict:
    ...
    dist_history = {field(row, "lth_distribution_observed_date") or field(row, "date"):
                    number(field(row, "lth_distribution")) for row in snapshots
                    if (field(row, "lth_distribution_observed_date") or field(row, "date")) <= as_of}
    ...
    for key, metric in METRICS.items():
        ...
        cards[key] = {"value": value, "date": day, "status": "OK" if row is not None else str(field(diagnostic, "status", "UNAVAILABLE")),
                      "reason": field(diagnostic, "error"), "source": field(diagnostic, "source"), "fetched_at": field(diagnostic, "fetched_at"),
    ...
    dist_day = dist_day or (field(latest_snapshot, "date") if latest_snapshot else None)
    ...
    state, reasons = core_state(values, coverage, minimum)
    lth_heat = threshold_label(values.get("lth_mvrv"), [(1, "CAPITULATION"), (2, "LOW_EARLY"), (3.5, "NORMAL_BULL"), (5, "HIGH"), (None, "EXTREME")])
    dist_pressure = threshold_label(distribution, [(40, "LOW"), (60, "NORMAL"), (70, "RISING"), (80, "HIGH"), (None, "EXTREME")])
    high, _, extreme = sell_side_flags(values.get("sell_side_risk"), values.get("sell_side_percentile_4y"))
    ...
    return {"state": state, "reasons": reasons, "observation_date": observation, "cards": cards,
            "substates": {"short_term_health": short_term_state(values.get("sth_mvrv"), values.get("sth_sopr")),
            ...
            "alerts": alerts(current, histories)}
